...
import com.leo.common.cache.Exception.LockException;
import com.leo.common.cache.annotation.Lock;
...
/**
 * <p>
 * 分布式锁aop 参考https://gitee.com/os_johnny/redisson-spring-boot-starter.git
 * </p>
 *
 * @author ：Leo
 * @since ：2021-01-29 16:11
 */
@Aspect
@Order(-10)
@Slf4j
        ...
        switch (lockModel) {
            case FAIR:
                rLock = redissonClient
                        .getFairLock(getVauleBySpel(keys[0], parameterNames, args, lock.keyConstant()).get(0));
                break;
            case RED_LOCK:
            ...
            case REENTRANT:
                ...
                break;
            case READ:
                RReadWriteLock rwlock = redissonClient
                        .getReadWriteLock(getVauleBySpel(keys[0], parameterNames, args, lock.keyConstant()).get(0));
                rLock = rwlock.readLock();
                break;
            case WRITE:
                RReadWriteLock rwlock1 = redissonClient
                        .getReadWriteLock(getVauleBySpel(keys[0], parameterNames, args, lock.keyConstant()).get(0));
